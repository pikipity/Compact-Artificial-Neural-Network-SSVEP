...
ch1 = [47, 53, 54, 55, 56, 57, 60, 61, 62]
chList=[y for x in [ch1] for y in x]
...
timeLen_list = np.floor(np.arange(0.2,2+0.2,0.2)*fs)

if os.path.isfile('test_BetaDataset.mat'):
    print('-----------------------')
    print('Load old data')
    tmp=scipy.io.loadmat('test_BetaDataset.mat')
...
for timeLen_i in range(len(timeLen_list)):
    ...
    for test_run in range(len(train_block_list)):
        for sub in range(sub_no):
            ...
            fileName = 'sub_'+str(sub+1)+'_allch.mat'
            allData = scipy.io.loadmat(fileName)['data'] # filterbank, trial, block, channel, time
            tmp = allData[:,:,:,:,int(np.floor(start_t*fs)-1):allData.shape[-1]]
            oneTrialData = tf.constant(tmp[0,0,0,chList,0:int(timeLen)])
            ...
            cca_classifier = gen_ecca_model_filterbank(trial_no, 5, timeLen, len(chList), 10)
            del optimizer, loss_metric, test_loss_metric
            optimizer = keras.optimizers.SGD(learning_rate=current_learning_rate, momentum=current_momentum)
            loss_metric = keras.metrics.Mean() 
            test_loss_metric = keras.metrics.Mean()
            
            template_tmp = allData[:,:,:,chList,int(np.floor(start_t*fs)-1):allData.shape[-1]]
            template_tmp = template_tmp[:, 0:int(trial_no), train_block, 0:int(9), 0:int(timeLen)]
            ...
            for block_i in range(len(train_block)):
                ...
                template_train_tmp = allData[:,:,:,chList,int(np.floor(start_t*fs)-1):allData.shape[-1]]
                template_train_tmp = template_train_tmp[:, 0:int(trial_no), train_block_tmp, 0:int(9), 0:int(timeLen)]
            ...
            for epoch in range(epoch_no):
                ...
                for block_i in range(len(train_block)):
                    ...
                    if block_i==0:
                        trainX = allData[:,:,:,chList,int(np.floor(start_t*fs)-1):allData.shape[-1]]
                        trainX = trainX[:,randomTrial,block,:,0:int(timeLen)]
                        ...
                        trainX_tmp = allData[:,:,:,chList,int(np.floor(start_t*fs)-1):allData.shape[-1]]
                        trainX_tmp = trainX_tmp[:,randomTrial,block,:,0:int(timeLen)]
                ...
                for block_i in range(len(test_block)):
                    ...
                    testX = allData[:,:,:,chList,int(np.floor(start_t*fs)-1):allData.shape[-1]]
                    testX = testX[:,randomTrial,block,:,0:int(timeLen)]
            ...
            cca_classifier.save('test_BetaDataset/timelen'+str(timeLen_i)+'_run'+str(test_run)+'_sub'+str(sub)+'_model')
            ...
            store_var['loss_history']=loss_history # forget
            store_var['test_loss_history']=test_loss_history
            scipy.io.savemat('test_BetaDataset.mat',store_var)
